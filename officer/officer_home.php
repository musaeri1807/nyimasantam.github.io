	...
	<div class="container">
		...
		<div class="col-lg-12">
			<center>
				...
				if(!isset($_SESSION['officer_login']))	//check unauthorize user not access in "welcome.php" page
				{
				...
				if(isset($_SESSION['officer_login'])){
					 ...
					 echo "Welcome Officer, ".$row['field_username'];

					$branch = $_SESSION['officer_cabang'];								
					$select_stmt = $db->prepare("SELECT * FROM tblbranch WHERE field_branch_id=:idbranch");
					$select_stmt->execute(array(":idbranch"=>$branch));	
					$rows=$select_stmt->fetch(PDO::FETCH_ASSOC);

					 echo "<br>";
					 echo "Kantor Cabang Anda Adalah, ".$rows['field_branch_name']." ".$rows['field_branch_id'];
				}
