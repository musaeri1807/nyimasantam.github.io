...
if(isset($_REQUEST['btn_register'])) //button name "btn_register"
{
	...
	$angka		= strip_tags($_REQUEST['txt_angka']);	//textbox name "txt_password"
	$time		= date('Y-m-d');
	...
	if(empty($username)){
		...
		$errorMsg[] = "Password must be atleast 7 characters";	//check passowrd must be 6 characters
	}
	// else if(!preg_match("0-9",$angka) < 16){
	else if(strlen($angka) <> 12){
		$errorMsg[] = "Number must be atleast 16 characters";	//check passowrd must be 6 characters
	}
	else
	{	
		try
		{	
			$select_stmt=$db->prepare("SELECT Username, Email,Field_handphone  FROM tbluserloginnasbaah 
										WHERE Username=:uname AND Email=:uemail OR Field_handphone=:only" ); // sql select query
			...
			if($row["Username"]==$username){
				...
				$errorMsg[]="Sorry Field_handphone already exists";	//check condition email already exists 
			}			
			...
			else if(!isset($errorMsg)) //check no "$errorMsg" show then continue
			{
				...
				$insert_stmt=$db->prepare("INSERT INTO tbluserloginnasbaah	(Username,Email,Password,Field_handphone,Tanggal,Token) VALUES
																(:uname,:uemail,:upassword,:only,:tgl,:rtoken)"); 		//sql insert query					
	...
	<div class="container">
		...
		?>   
			...
			<form method="post" class="form-horizontal">
				...
				<div class="form-group">
				<label class="col-sm-3 control-label">Angka Only</label>
				<div class="col-sm-6">
				<input type="text" name="txt_angka" class="form-control" placeholder="Angka Only" />
				</div>
				</div>
