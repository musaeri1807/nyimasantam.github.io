	...
	if (empty($firstname)) {
		...
		try {
			if (!isset($errorMsg)) {
				...
				$update_stmt = $db->prepare('UPDATE tblemployeeslogin SET field_name_officer=:uname,field_gender=:ugender,field_role=:urole, field_branch=:cabang, field_token=:token, field_status_aktif=:statuse WHERE field_user_id=:id');
				$update_stmt->bindParam(':id', $id);
				// $update_stmt->bindParam(':idemployee',$idemployee);
				$update_stmt->bindParam(':uname', $firstname);
				$update_stmt->bindParam(':ugender', $gender);
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								<select class="form-control" type="text" name="txt_gender" id="gender">
									<option value="<?php echo $row["field_gender"]; ?>">
										<?php
										if ($row["field_gender"] == "L") {
											echo "LAKI-LAKI";
										} else if ($row["field_gender"] == "P") {
											echo "PEREMPUAN";
										} else {
											echo "PILIH";
										}
						...
						<div class="form-group">
							...
							<div class="col-sm-3">
								<select class="form-control" type="text" name="txt_role">
									<option value="<?php echo $row["field_role"]; ?>"><?php echo $row["field_role"] . "-";
																						echo $row['field_department_name']; ?></option>
									<?php foreach ($resultdept as $rows) { ?>
										<option value="<?php echo $rows['field_department_id']; ?>"><?php echo $rows['field_department_id'] . "-";
																									echo $rows['field_department_name']; ?></option>
									<?php } ?>
								</select>
							</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								<select class="form-control" type="text" name="txt_cabang">
									<option value="<?php echo $row["field_branch"]; ?>">
										<?php echo $row['field_branch_name'] . "-";
										echo $row["field_branch"]; ?>
									</option>
